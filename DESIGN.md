...
components:
  ...
  action-project:
    role: 'Project-card affordance only. Now opens the internal case study at /work/[slug].'
    ...
    hover: 'plate-turn — rotate3d(0,1,0.05,-2.4deg) + mint specular sweep + oversized ghost "Read" bleed-through'
  action-panel:
...
| **Project**   | Project-card affordance only. Opens the internal case study at `/work/[slug]`. | Circle with rotating textPath ring + plane orbit, 8s spin (motion-safe).                             | **Plate-Turn.** The card catches light like a book page turning: `rotate3d(0, 1, 0.05, -2.4deg)` around a right-center pivot with `perspective: 1500px`, a mint-tinted specular sweep passes across the plate, and an oversized ghost "Read" watermark bleeds through from underneath at low opacity (`mix-blend-mode: soft-light`). Reads as "the next page is showing through," distinct from Primary's outward radial expansion. Reduced-motion: crossfade the ghost only, no rotation, no sweep. | Every project card. Never used outside the project-card domain.     |
...
**The Reserved-Hover Rule.** Each tier owns its hover signature exclusively: Primary owns radial expansion (`scale-150`), Secondary owns the bubble-particle keyframes, Tertiary owns underline reveal, Project owns the plate-turn (3D page-catches-light rotation + mint specular sweep + ghost "Read" bleed-through), Panel owns the underline weight shift. Mystery Box is the sixth voice — playful physics (`scale-1.25 rotate-3`) — and is not a tier. When you add a new interactive surface, choose a tier and inherit its hover; do not compose a new hover per-component.

The Project tier previously used a directional-lift (`translate-y-1 scale-105`) hover; that gesture was retired once project cards began opening internal case studies. Directional-lift read as "come toward the destination" — meaningful when the destination was an off-site URL, but the Plate-Turn better matches the new semantic ("open the plate; read what's underneath").
